...
class SVD_LlamaAttention(nn.Module):
    ...
    # This version stores low-rank KV cache instead.
    def forward(
        self,
        hidden_states,
        attention_mask=None,
        ...
        **kwargs,
        ...
        # 1. Query remains full (Query is not cached)
        query_states = self.q_v_proj(self.q_u_proj(hidden_states))
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)

        # Precompute RoPE for current tokens if available
        ...
        # 2. KV low-rank computation (only u_proj)
        # Shape is [bsz, q_len, rank]
        latent_key = self.k_u_proj(hidden_states)
        latent_value = self.v_u_proj(hidden_states)

        # 3. KV cache adaptation (store low-rank latent tensors)
        if past_key_values is not None:
            # Case A: new DynamicCache object (transformers default)
            if hasattr(past_key_values, "update"):
                # update handles concatenation and returns updated latent_key/value
                cache_kwargs = {}
                if cos_q is not None and sin_q is not None:
                    cache_kwargs["sin"] = sin_q
                    cache_kwargs["cos"] = cos_q
                if cache_position is not None:
                    cache_kwargs["cache_position"] = cache_position
                latent_key, latent_value = past_key_values.update(
                    latent_key, latent_value, self.layer_idx, cache_kwargs
                )
            # Case B: old tuple format
            elif isinstance(past_key_values, tuple):
                # past_key_values[0] stores previous latent_key
                latent_key = torch.cat([past_key_values[0], latent_key], dim=1) 
                latent_value = torch.cat([past_key_values[1], latent_value], dim=1)
                past_key_values = (latent_key, latent_value)
        else:
            # Prefill: initialize cache if use_cache is enabled
            if use_cache:
                past_key_values = (latent_key, latent_value)

        # --- Print stats (key fix: inspect latent_key directly) ---
        if use_cache:
            # latent_key now includes past + current and is low-rank
            # Shape is usually [bsz, total_seq_len, rank]
            current_shape = latent_key.shape 
            
            # Compute memory usage (MB)
            element_size = latent_key.element_size() 
            # Sum key + value tensor sizes
            total_mem_mb = (latent_key.nelement() + latent_value.nelement()) * element_size / (1024**2)
            
            # Print only layer 0 to avoid spam
            if self.layer_idx == 0:
                print(f"\n[Layer {self.layer_idx}] SVD-KV Cache Info:")
                print(f" - Latent Shape: {list(current_shape)} (Rank: {current_shape[-1]})")
                print(f" - Memory Usage: {total_mem_mb:.4f} MB")

        # 4. Expand back (compute phase)
        # Use v_proj to restore to [bsz, total_len, num_heads * head_dim]
        key_states = self.k_v_proj(latent_key)
        value_states = self.v_v_proj(latent_value)

        # Reshape to multi-head format
        total_len = key_states.shape[1]
        ...
        # 5. Apply RoPE
        # RoPE must be applied after expansion. Query/Key lengths can differ during decoding,
        # so position ids must cover the full cache length for keys.
        if self.rotary_emb is not None:
            if total_len == q_len:
                if cos_q is None or sin_q is None:
                    cos_q, sin_q = self.rotary_emb(query_states, query_position_ids)
                ...
                if cos_q is None or sin_q is None:
                    cos_q, sin_q = self.rotary_emb(query_states, query_position_ids)
                cos_k, sin_k = self.rotary_emb(value_states, key_position_ids)

                query_states, _ = self._apply_rotary_pos_emb(query_states, query_states, cos_q, sin_q)
                _, key_states = self._apply_rotary_pos_emb(key_states, key_states, cos_k, sin_k)
...
def apply_svd_compression(model, svd_list, ratio=None, rank_allocation=None, save_loss_path=None):
    ...
    for i in tqdm(range(len(layers)), desc="Compressing Layers"):
        ...
        if rank_allocation is not None:
            ranks = {k.split('_')[0]: v for k, v in rank_allocation[i].items() if 'rank' in k}
        else:
